...
def read_single_key(prompt="請選擇: "):
    """讀取單一按鍵 (改為 input 以支援中文輸入並統一體驗)"""
    try:
        val = input(prompt).strip()
        # 如果用戶輸入多個字元，只取第一個（為了相容舊邏輯）
        # 但如果是空字串（直接按 Enter），返回空
        if not val:
            return ""
        return val[0]
    except EOFError:
...
class InstitutionalInvestorAPI:
    ...
    def fetch_twse_openapi(cls, progress=None):
        """從 TWSE OpenAPI 取得今日法人買賣超資料"""
        results = []
        try:
            if progress:
                progress.info("正在從 TWSE OpenAPI 取得法人資料...", level=1)
            ...
            resp = requests.get(cls.TWSE_OPENAPI_URL, headers=headers, timeout=30, verify=False)
            resp.raise_for_status()
            
            # 處理 API 返回空內容的情況（凌晨時段 OpenAPI 尚未更新）
            content = resp.text.strip()
            if not content or content == '[]':
                if progress:
                    progress.info("TWSE 法人 OpenAPI 尚無今日資料 (可能尚未更新)", level=1)
                return results
            
            data = resp.json()
            if not isinstance(data, list):
                logger.warning("TWSE 法人 OpenAPI 回傳非陣列格式")
                return results
            ...
            logger.error(f"TWSE 法人 OpenAPI 失敗: {e}")
            if progress:
                progress.error(f"✗ TWSE 法人 OpenAPI 失敗: {e}", level=1)
...
class MarginDataAPI:
    ...
    def fetch_twse_margin(cls, progress=None):
        """
        從 TWSE OpenAPI 取得融資融券資料
        回傳: list of dict
        """
        results = []
        try:
            if progress:
                progress.info("正在下載 TWSE 融資融券資料 (OpenAPI)...", level=1)
            ...
            data = resp.json()
            if not isinstance(data, list):
                logger.warning("TWSE 融資融券 API 回傳非陣列格式")
                return results
            ...
            for item in data:
                ...
                record = {
                    ...
                    "margin_util_rate": 0.0,  # TWSE API 不直接提供
                    "short_util_rate": 0.0,
                    "market": "TWSE"
                }
            ...
            if progress:
                progress.success(f"✓ TWSE 融資融券: {len(results)} 筆", level=1)
            
        except requests.exceptions.RequestException as e:
            logger.error(f"TWSE 融資融券 API 請求失敗: {e}")
            if progress:
                progress.error(f"✗ TWSE 融資融券 API 失敗: {e}", level=1)
        except Exception as e:
            logger.error(f"TWSE 融資融券資料解析失敗: {e}")
            if progress:
                progress.error(f"✗ TWSE 融資融券解析失敗: {e}", level=1)
...
class PePbDataAPI:
    ...
    def fetch_twse_pepb(cls, progress=None):
        """從 TWSE OpenAPI 取得今日 PE/PB 資料"""
        results = []
        try:
            if progress:
                progress.info("正在從 TWSE OpenAPI 取得 PE/PB 資料...", level=1)
            ...
            data = resp.json()
            if not isinstance(data, list):
                logger.warning("TWSE PE/PB OpenAPI 回傳非陣列格式")
                return results
            ...
            if progress:
                progress.success(f"✓ TWSE PE/PB: {len(results)} 筆", level=1)
                
        except Exception as e:
            logger.error(f"TWSE PE/PB OpenAPI 失敗: {e}")
            if progress:
                progress.error(f"✗ TWSE PE/PB OpenAPI 失敗: {e}", level=1)
...
class ShareholderDataAPI:
    ...
    def fetch_all_shareholder(cls, progress=None):
        """取得所有集保戶數資料 (FinMind 優先，TDCC 備援)"""
        # 先嘗試 FinMind
        data = cls.fetch_from_finmind(progress)
        
        # 若 FinMind 失敗或無資料，使用 TDCC CSV
        if not data:
            if progress:
                progress.info("FinMind 無資料，切換至 TDCC CSV...", level=1)
...
def get_color_code(val):
    """Get color code based on value (Red for positive, Green for negative)"""
...
def get_volume_color(ratio):
    ...
    if ratio >= 2.0: return Colors.MAGENTA  # Explosion
    elif ratio >= 1.2: return Colors.RED    # High
    elif ratio <= 0.6: return Colors.GREEN  # Low
    return Colors.RESET
...
def format_scan_result(code, name, indicators, show_date=False):
    ...
    colored_chg14 = get_colored_value(f"{chg14:.1f}%", chg14 - chg14_prev, chg14_arrow)
    ...
    # Line 2
    close_color = get_trend_color(close, close_prev)
...
def get_volume_color(ratio):
    """根據成交量比值獲取顏色 (Table-Driven)"""
    # Table-Driven: (Threshold, Color)
...
def step1_fetch_stock_list():
    ...
    # 1. TWSE 上市 (OpenAPI) - BWIBBU_ALL (上市個股日本益比、殖利率及股價淨值比)
    # 這個 API 通常包含 Code, Name, PE, DY, PB... 但不一定有上市日期
    # 改用 https://openapi.twse.com.tw/v1/exchangeReport/STOCK_DAY_ALL (個股日成交資訊) 也不含日期
    # 為了取得上市日期，我們可能需要查閱 https://openapi.twse.com.tw/v1/opendata/t187ap03_L (上市公司基本資料)
    
    try:
        # 上市公司基本資料 (含上市日期)
        url_meta = "https://openapi.twse.com.tw/v1/opendata/t187ap03_L"
        print_flush("  -> 下載 TWSE 基本資料 (OpenAPI)...", end="")
    ...
    try:
        ...
        print_flush("  -> 下載 TPEx 基本資料 (OpenAPI)...", end="")
        headers = {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36"
        }
    ...
    if stocks:
        ...
        # Fallback: Try to fill missing list_date using twstock
        print_flush("  -> 正在補全上市日期 (使用 twstock)...", end="")
        filled_count = 0
        for idx, row in df.iterrows():
            if not row['list_date']:
                try:
                    stock_info = twstock.codes.get(row['code'])
                    if stock_info and stock_info.start:
                        # twstock start format: YYYY/MM/DD
                        l_date = stock_info.start.replace('/', '-')
                        df.at[idx, 'list_date'] = l_date
                        filled_count += 1
                except:
                    pass
        print_flush(f" ✓ (補全 {filled_count} 筆)")
        
        df.to_csv(STOCK_LIST_PATH, index=False)
        print_flush(f"✓ 已更新 {len(df)} 檔股票至清單")
        
        # 更新 stock_meta 資料庫
        try:
            with db_manager.get_connection() as conn:
                cur = conn.cursor()
                cur.execute("DELETE FROM stock_meta") # 重建白名單
                
                # Schema: code, name, list_date, delist_date, market_type
                # Note: df to list of tuples
                data_to_insert = []
                for _, row in df.iterrows():
                    data_to_insert.append((row['code'], row['name'], row['market'], row['list_date']))
                
                cur.executemany("""
                    INSERT INTO stock_meta (code, name, market_type, list_date, delist_date)
                    VALUES (?, ?, ?, ?, '')
                """, data_to_insert)
            ...
            print_flush(f"⚠ 資料庫同步失敗: {e}")

        # 同步至 Supabase
        if ENABLE_CLOUD_SYNC:
            # sync_stock_names_to_supabase(stocks)
            pass
    else:
        print_flush("❌ 更新失敗: 無法獲取任何股票資料")
...
# ==============================
def update_market_data(market_name, fetch_func, parse_func):
    """
...
def _fetch_twse_data():
    # OpenAPI 不需要 cookie 也不需要複雜 headers，但加 User-Agent 較保險
    headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36'
    }
    ...
    try:
        ...
        if updates:
            with db_manager.get_connection() as conn:
                ...
                """, updates)
                conn.commit()
            print_flush(f"✓ 已更新 {len(updates)} 筆 TPEx 估值資料")
    except Exception as e:
        print_flush(f"❌ TPEx 估值更新失敗: {e}")

def step2_download_tpex_daily():
    """步驟2: 下載 TPEx (上櫃) 本日行情 (含估值)"""
    updated = update_market_data("TPEx (上櫃)", _fetch_tpex_data, _parse_tpex_item)
    ...
    try:
        ...
        if updates:
            with db_manager.get_connection() as conn:
                ...
                """, updates)
                conn.commit()
            print_flush(f"✓ 已更新 {len(updates)} 筆 TWSE 估值資料")
    except Exception as e:
        print_flush(f"❌ TWSE 估值更新失敗: {e}")

def step3_download_twse_daily():
    """步驟3: 下載 TWSE (上市) 本日行情 (含估值)"""
    updated = update_market_data("TWSE (上市)", _fetch_twse_data, _parse_twse_item)
...
def step4_check_data_gaps():
    ...
    with db_manager.get_connection() as conn:
        ...
        sql = """
            SELECT code, 
                   COUNT(*) as total_cnt,
                   SUM(CASE WHEN amount IS NULL OR amount = 0 THEN 1 ELSE 0 END) as missing_amount_cnt
            FROM stock_history 
            GROUP BY code
        """
    ...
    if not count_gaps and not amount_gaps and not inst_gaps:
        print_flush(f"✓ 所有股票資料皆充足 (>= {MIN_DATA_COUNT} 筆, 金額/法人皆完整)")
    else:
        if count_gaps:
...
def step3_5_download_institutional(days=60):
    ...
    try:
        ...
        try:
            ...
            if saved > 0:
                print_flush(f"✓ 官方 OpenAPI: 已儲存 {saved} 筆今日法人資料")
        ...
        # 3. 找出缺漏日期
        missing_dates = [d for d in dates_to_check if int(d.strftime("%Y%m%d")) not in existing_dates]
        ...
        print_flush(f"✓ 已更新 {count} 檔股票的大戶持股比例與總股東人數")
        
    except Exception as e:
        print_flush(f"❌ 法人資料下載失敗: {e}")

def step3_6_download_major_holders(force=False):
    """步驟3.6: 下載集保戶股權分散表 (千張大戶持股比例 & 總股東人數) - 每週五更新"""
    print_flush("\n[Step 3.6] 下載集保戶股權分散表 (千張大戶 & 總股東人數)...")
    
    # 檢查是否為週五 (weekday 4) 或資料缺失
    should_run = force or datetime.now().weekday() == 4
    ...
    if not should_run:
        print_flush("⚠ 非週五且資料完整，跳過集保大戶資料下載 (每週五更新)")
        return
...
def step3_7_fetch_margin_data(days=60):
    ...
    try:
        ...
        try:
            ...
            if saved > 0:
                print_flush(f"✓ 官方 OpenAPI: 已儲存 {saved} 筆今日融資融券資料")
                openapi_success = True
        except Exception as e:
            print_flush(f"⚠ 官方 OpenAPI 失敗: {e}，切換至備援來源...")
        ...
        # 3. 找出缺漏日期 (若今天已由 OpenAPI 成功取得則跳過)
        missing_dates = []
        for d in dates_to_check:
            d_int = int(d.strftime("%Y%m%d"))
            if d_int not in existing_dates:
                if d_int == today_int and openapi_success:
                    continue  # 今天已由 OpenAPI 取得
...
def step3_8_fetch_market_index(date_str=None):
    ...
    if records:
        try:
            with db_manager.get_connection() as conn:
                ...
                """, records)
                conn.commit()
            print_flush(f"✓ 已更新大盤指數資料 ({len(records)} 筆)")
        except Exception as e:
    ...
    with db_manager.get_connection() as conn:
        ...
        cur.execute(f"""
            SELECT code, COUNT(*), MIN(date_int), MAX(date_int),
                   SUM(CASE WHEN (amount IS NULL OR (amount = 0 AND volume > 0)) AND date_int >= {cutoff_int} THEN 1 ELSE 0 END)
            FROM stock_history 
            GROUP BY code
        """)
...
def scan_mfi_mode(indicators_data, order='asc', min_volume=0):
    ...
    for code, ind in indicators_data.items():
        ...
        if order == 'asc':
            if mfi > mfi_prev and mfi < 30:
                results.append((code, mfi, ind))
        else:
            if mfi < mfi_prev and mfi > 70:
                results.append((code, mfi, ind))
...
def scan_smart_money_strategy():
    ...
    codes = display_scan_results(results, "聪明錢掃描結果 (NVI版)", limit=limit, description="聪明錢: 基於 NVI 趨勢、NVI/PVI 關係、無背離、價格趨勢、無量價背離、週線 NVI 之綜合評分 (滿分6分)")
...
def execute_kd_golden_scan():
    ...
    codes = display_scan_results(results, "月KD交叉", limit=limit, extra_info_func=kd_extra, description="KD: 隨機指標, K值由下往上穿越D值=黃金交叉 (買進訊號)")
    prompt_stock_detail_report(codes)
...
def scan_nvi_pvi_crossover():
    ...
    def nvi_extra(code, val, ind, sigs):
        return f"[{sigs}] NVI:{val:.1f} PVI:{ind.get('pvi', 0):.1f}"
        
    # 轉換格式以符合 display_scan_results
    final_results = [(r[0], r[1], r[2]) for r in results]
    # Hack: 把 signal 字串塞回 ind 以便 extra_func 使用 (或者 display_scan_results 支援 4-tuple?)
    # display_scan_results 只接受 (code, sort_val, ind)
    for r in results:
        r[2]['_scan_note'] = r[3]
        
    def nvi_extra_wrapper(code, val, ind):
        return nvi_extra(code, val, ind, ind.get('_scan_note', ''))

    codes = display_scan_results(final_results, "NVI/PVI 交叉掃描", limit=limit, extra_info_func=nvi_extra_wrapper, description="NVI: 負量指標 (聰明錢), PVI: 正量指標 (散戶). NVI穿越PVI=主力控盤")
    prompt_stock_detail_report(codes)
...
def scan_ma_alignment_rising(check_price_above=True):
    ...
    def ma_extra(code, dist_pct, ind):
        ...
        ma_values = {'WMA20': wma20, 'WMA60': wma60, 'WMA120': wma120, 'WMA200': wma200}
        highest_ma_name = max(ma_values, key=lambda k: ma_values[k] if ma_values[k] else 0)
        
        wma_line = f"\nWMA20:{wma20:.2f} WMA60:{wma60:.2f} WMA120:{wma120:.2f} WMA200:{wma200:.2f} [距{highest_ma_name}:{dist_pct:.1f}%]"
        return wma_line
    
    codes = display_scan_results(results, title, limit=limit, extra_info_func=ma_extra, description="條件: 20,60,120,200 全數向上，股價 > 所有均線，距最高均線 0-10%")
    prompt_stock_detail_report(codes)
...
def scan_pv_divergence_analysis():
    ...
    print_flush("\n" + "="*90)
    print_flush(f"{'代號':<6} {'名稱':<8} {'收盤':<10} {'型態':<10} {'強度':<8} {'風險':<8} {'建議':<20}")
    print_flush("-" * 90)
    ...
    for res in results:
        # Map internal fields to display fields
        # res['type'] is "價跌量漲" or "價漲量跌" -> Map to "NVI背離" etc if needed, or keep as is.
        # Proposal says "型態" e.g. "NVI背離".
        # Current logic uses 3-day divergence. Let's use the type we found.
        
        # Strength: score 0-1. Map to stars.
        stars = "★" * int(res['score'] * 3)
        if not stars: stars = "★"
        
        # Risk: Use MA200 trend or calculate something else. 
        # Proposal example: "低"
        # Let's use MA200 trend as proxy: Up=Low Risk, Down=High Risk
        risk = "低" if res['ma200_trend'] == "上揚" else "高"
        
        # Suggestion
        sugg = res['suggestion'].split("，")[0] # Shorten
        
        c_price = Colors.RED # Default
        price_str = f"{c_price}{safe_float_preserving_none(res['info'].get('close')):.2f}{reset}"
        
        print_flush(f"{res['code']:<6} {res['name']:<8} {price_str:<19} {res['type']:<12} {stars:<10} {risk:<10} {sugg:<20}")
        if count >= limit: break
        
        nvi_str = "多頭" if res['nvi_sig'] else "空頭"
        sugg = f"止盈:{res['tp']:.2f}/止損:{res['sl']:.2f}"
        
        print_flush(f"{res['code']:<6} {res['name']:<6} {res['color']}{res['type']:<10}{reset_color()} {res['ma200_trend']:<8} {nvi_str:<6} {sugg}")
        display_codes.append(res['code'])
        count += 1
        
    print_flush("="*80)
    print_flush(f"[找到 {len(results)} 檔，顯示前 {count} 檔]")
    
    if not results:
        print_flush("💡 提示: 若無結果，請執行 [1] 資料管理 -> [7] 計算技術指標 以更新背離信號")
        return
    
    # Prompt for detail report
...
def check_db_nulls():
    ...
    try:
        with db_manager.get_connection() as conn:
            ...
            print_flush("2. 融資融券/大盤指數應有 450 天以上資料。")
...
def scan_vsbc_strategy():
    ...
    print_flush(f"\n【多方行為 TOP】 (前 {limit} 筆)")
    # Header: 代號 名稱 收盤 ma20(乖離%) 成交量 60mv(倍數) VSBC上/下 VP上/下
    header = f"{'代號':<6} {'名稱':<8} {'收盤':<10} {'ma20(乖離%)':<14} {'成交量':<10} {'60mv(倍數)':<12} {'VSBC上/下':<10} {'VP上/下':<10}"
    print_flush(header)
    print_flush("-" * 90)
    ...
    for res in results[:limit]:
        ...
        # Format MA20 Bias
        bias_str = f"{res['ma20_bias']:+.2f}%"
        
        # Format Volume
        vol_str = f"{int(res['volume']/1000):,}" # In thousands (張)
        
        # Format Vol Ratio
        ratio_str = f"{res['vol_ratio']:.1f}"
        
        # VSBC Status
        vsbc_status = "中性區"
        if res['vsbc_pct'] >= 80: vsbc_status = "壓力區"
        elif res['vsbc_pct'] <= 20: vsbc_status = "支撐區"
        
        # VP Status (Mock logic as we only have POC)
        vp_status = "中性區"
        if res['close'] > res['poc'] * 1.02: vp_status = "壓力區"
        elif res['close'] < res['poc'] * 0.98: vp_status = "支撐區"
        
        print_flush(f"{res['code']:<6} {res['name']:<8} {price_str:<19} {bias_str:<14} {vol_str:<10} {ratio_str:<12} {vsbc_status:<10} {vp_status:<10}")
...
def scan_candlestick_patterns():
    ...
    header = f"{'代號':<6} {'名稱':<8} {'收盤':<10} {'成交量(量能比)':<16} {'VSBC上/下':<16} {'費波那契回撤':<12}"
    reset = reset_color()
    ...
    def print_table(title, data):
        ...
        print_flush(header)
        print_flush("-" * 90)
        for res in data[:limit]:
            c_price = get_trend_color(res['close'], res['close_prev'])
            price_str = f"{c_price}{res['close']:.2f}{reset}"
            
            vol_str = f"{res['vol_ratio']:.1f}倍"
            
            # VSBC Status: Show VSBC/POC
            # User request: "0000/0000"
            vsbc_str = f"{res['vsbc_val']:.0f}/{res['poc']:.0f}"
            
            # Fib: Show current retracement ratio and price
            # User request: "費波那契回撤...應該會有比例"
            # We compare current_ratio to standard levels: 0.236, 0.382, 0.5, 0.618, 0.786
            ratio = res['fib_ratio']
            levels = [0.236, 0.382, 0.5, 0.618, 0.786]
            nearest = min(levels, key=lambda x: abs(x - ratio))
            
            if abs(ratio - nearest) < 0.05:
                fib_str = f"{nearest}({res['close']:.0f})"
            else:
                fib_str = f"{ratio:.2f}({res['close']:.0f})"
            
            print_flush(f"{res['code']:<6} {res['name']:<8} {price_str:<19} {vol_str:<21} {vsbc_str:<19} {fib_str:<12}")

    print_table("早晨之星 (底部反轉)", morning_stars)
    print_table("黃昏之星 (頂部反轉)", evening_stars)
...
def scan_vp(indicators_data, mode='lower', min_volume=100):
    ...
    for code, ind in indicators_data.items():
        try:
            ...
            if vol < min_volume * 1000: # Convert to shares
                continue
            ...
            if mode == 'lower':
                ...
                if abs(close - vp_lower) / close < 0.02:
...
def scan_ma_bullish():
    ...
    print_flush(f"\n【均線多頭排列 (乖離0-10%)】 (前 {limit} 筆)")
    print_flush(f"{'代號':<6} {'名稱':<8} {'現價':<8} {'乖離%':<8} {'成交量':<8}")
    print_flush("-" * 50)
    for res in results[:limit]:
        print_flush(f"{res['code']:<6} {res['name']:<8} {res['close']:<8.2f} {res['bias']:<8.2f} {int(res['volume']):<8}")
...
def scan_five_filter():
    ...
    print_flush(f"\n【五階篩選 TOP】 (前 {limit} 筆)")
    header = f"{'代號':<6} {'名稱':<8} {'收盤':<10} {'分數':<8} {'RS值':<8} {'K值':<8}"
    print_flush(header)
    print_flush("-" * 60)
    ...
    for res in results[:limit]:
        ...
        # Score Color
        c_score = Colors.RED if res['score'] >= 80 else (Colors.MAGENTA if res['score'] >= 70 else reset)
        score_str = f"{c_score}{res['score']:.1f}{reset}"
        
        print_flush(f"{res['code']:<6} {res['name']:<8} {price_str:<19} {score_str:<17} {res['rs']:<8.2f} {res['k']:<8.1f}")
...
def scan_six_dim_resonance():
    ...
    print_flush(f"\n【六維共振 (至少5項符合)】 (前 {limit} 筆)")
    # Header: 代號 名稱 收盤 訊號 維度 MACD KDJ RSI LWR BBI MTM
    print_flush(f"{'代號':<6} {'名稱':<8} {'收盤':<10} {'訊號':<6} {'維度':<6} {'MACD':<5} {'KDJ':<5} {'RSI':<5} {'LWR':<5} {'BBI':<5} {'MTM':<5}")
    print_flush("-" * 90)
    
    reset = reset_color()
    for res in results[:limit]:
        c_price = Colors.RED if res['close'] > 0 else Colors.GREEN # Simple check, usually red for price
        price_str = f"{c_price}{res['close']:.2f}{reset}"
        
        signal_str = f"{Colors.RED}買入{reset}"
        dim_str = f"{res['score']}/6"
        
        # Details checkmarks
        d = res['details']
        # d is [cond_macd, cond_kdj, cond_rsi, cond_lwr, cond_bbi, cond_mtm]
        checks = [f"{Colors.RED}✓{reset}" if x else " " for x in d]
        
        print_flush(f"{res['code']:<6} {res['name']:<8} {price_str:<19} {signal_str:<15} {dim_str:<8} {checks[0]:<14} {checks[1]:<14} {checks[2]:<14} {checks[3]:<14} {checks[4]:<14} {checks[5]:<14}")
    
    print_flush("\n指標說明: 1.MACD>Sig 2.K>D 3.RSI>50 4.LWR>-50 5.Px>BBI 6.MTM>0")
...
def vp_scan_submenu():
    ...
    if ch in ['1', '2']:
        ...
        codes = display_scan_results(res, title, limit=limit, description="VP: Volume Profile (籌碼分布), 接近下緣=支撐, 接近上緣=壓力")
        prompt_stock_detail_report(codes)
...
def mfi_scan_submenu():
    ...
    if ch in ['1', '2']:
        ...
        codes = display_scan_results(results, title, limit=limit, description="MFI: Money Flow Index (資金流量), >80 超買, <20 超賣, 50 分界")
        prompt_stock_detail_report(codes)
...
def ma_scan_submenu():
    ...
    if ch in ['1', '2']:
        ...
        codes = display_scan_results(results, title, limit=limit, description="MA: Moving Average (移動平均線), 股價低於均線=回測或跌破")
        prompt_stock_detail_report(codes)
...
def _run_full_daily_update():
    ...
    # Step 3.5: 下載法人資料 (智慧補漏: 檢查近 60 天)
    step3_5_download_institutional(days=60)
    
    # Step 3.6: 下載集保大戶
    step3_6_download_major_holders()
    
    # Step 3.7 & 3.8: 下載融資融券與大盤指數 (檢查近 60 天)
    step3_7_fetch_margin_data(days=60)
    step3_8_fetch_market_index()
    
    # Step 3.9: 下載 PE/PB 估值資料 (官方 OpenAPI)
    print_flush("\n[Step 3.9] 下載 PE/PB 估值資料 (官方 OpenAPI)...")
    try:
        saved = PePbDataAPI.fetch_all_pepb()
        if saved > 0:
            print_flush(f"✓ PE/PB 資料: {saved} 筆")
    except Exception as e:
        print_flush(f"⚠ PE/PB 下載失敗: {e}")
    
    # Step 3.10: 下載集保戶數資料 (FinMind 優先，TDCC 備援)
    print_flush("\n[Step 3.10] 下載集保戶數資料 (FinMind/TDCC)...")
    try:
        saved = ShareholderDataAPI.fetch_all_shareholder()
        if saved > 0:
            print_flush(f"✓ 集保戶數: {saved} 筆")
    except Exception as e:
        print_flush(f"⚠ 集保戶數下載失敗: {e}")
    
    # Step 4 & 5: 檢查與清理
    step4_check_data_gaps()
    step5_clean_delisted()
    
    # Step 6: 補漏 (只載入一次，傳遞給後續步驟)
    data = step4_load_data()
    s6 = step6_verify_and_backfill(data, resume=True)
    if isinstance(s6, set):
        updated_codes.update(s6)
    
    # Step 7: 計算指標 (使用已載入的 data)
    ...
    print_flush("✓ 更新完成！快取已更新，可直接進行掃描")
...
def _check_api_connection_status():
    """檢查各 API 連線狀態"""
    print_flush("\n" + "="*60)
    print_flush("【API 連線狀態檢查】")
    print_flush("="*60)
    
    apis = [
        ("TWSE 證交所", "https://www.twse.com.tw/rwd/zh/afterTrading/STOCK_DAY?response=json"),
        ("TPEx 櫃買中心", "https://www.tpex.org.tw/openapi/v1/tpex_mainboard_daily_close_quotes"),
    ...
    for name, url in apis:
        try:
            ...
            if resp.status_code == 200:
                print_flush(f"✓ {name}: 連線正常 (HTTP {resp.status_code})")
            else:
                print_flush(f"⚠ {name}: HTTP {resp.status_code}")
        except requests.exceptions.Timeout:
            print_flush(f"❌ {name}: 連線逾時")
    ...
    # 檢查 Supabase
    if ENABLE_CLOUD_SYNC and HAS_SUPABASE:
        try:
            supabase = create_client(SUPABASE_URL, SUPABASE_KEY)
            supabase.table("stock_data").select("code").limit(1).execute()
            print_flush(f"✓ Supabase 雲端: 連線正常")
        except Exception as e:
            print_flush(f"❌ Supabase 雲端: {e}")
    else:
        print_flush(f"⚠ Supabase 雲端: 未啟用或缺少套件")
...
def maintenance_menu():
    ...
    # 表驅動法：維護功能映射
    MAINTENANCE_MENU_ACTIONS = {
        '1': backup_menu,
        '2': _check_api_connection_status,
        '3': check_db_nulls,
        '4': lambda: (print_flush("\n[INFO] 開始重新計算所有股票的指標..."), step7_calc_indicators(force=True)),
        '5': step8_sync_supabase # [Mod] 改為同步到 Supabase
    }
    ...
    while True:
        ...
        # 表驅動法：查找並執行
        action = MAINTENANCE_MENU_ACTIONS.get(choice)
        if action:
            action()
...
def institutional_menu():
    ...
    def _display_ranking(rank_type, title, top_n, min_days, sort_by_amount):
        ...
        try:
            with db_manager.get_connection() as conn:
                ...
                sql = f"""
                    SELECT code, {order_col} as net_buy
                    FROM institutional_investors
                    WHERE date_int = ?
                    ORDER BY net_buy {order_dir}
                    LIMIT 100
                """
                cur = conn.execute(sql, (latest_date,))
                data = [{'stock_id': row[0], 'net_buy': row[1]} for row in cur.fetchall()]
                ...
                stock_names = {}
                try:
                    cur2 = conn.execute("SELECT code, name FROM stock_meta")
                    stock_names = {row[0]: row[1] for row in cur2.fetchall()}
                except:
                    pass
                
                # 計算連續買賣超天數
                def get_consecutive_days(code, col_name, is_buy):
                    """計算連續買入或賣出天數"""
                    try:
                        cur3 = conn.execute(f"""
                            SELECT date_int, {col_name} as net
                            FROM institutional_investors
                            WHERE code = ?
                            ORDER BY date_int DESC
                            LIMIT 60
                ...
                for i, row in enumerate(data[:top_n], 1):
                    code = row['stock_id']
                    net_lots = row['net_buy']
                    close = close_prices.get(code, 0)
                    amount = (net_lots * 1000 * close) / 10000 if close else 0  # 萬元
                    name = stock_names.get(code, '')
                    display_name = f"{name}({code})" if name else code
                    
                    # 計算連續天數
                    consec_days = get_consecutive_days(code, order_col, is_buy)
                    
                    print_flush(f"{i:<3} {display_name:<16} | {net_lots:>10,} | {amount:>10,.0f} | {consec_days:>8}")
    ...
    while True:
        ...
        print_flush("【法人買賣超排行】")
        print_flush("="*60)
        print_flush("[1] 外資買超排行 (連續買入天數由大排到小)")
        print_flush("[2] 投信買超排行 (連續買入天數由大排到小)")
        print_flush("[3] 自營商買超排行 (連續買入天數由大排到小)")
        print_flush("[4] 外資賣超排行 (連續賣出天數由大排到小)")
        print_flush("[5] 投信賣超排行 (連續賣出天數由大排到小)")
        print_flush("[6] 自營商賣超排行 (連續賣出天數由大排到小)")
        print_flush("[0] 返回主選單")
        print_flush("-" * 60)
        print_flush("💡 輸入股票代號 (如 2330) 可直接查看個股")
        ...
        if ch in rank_map:
            ...
            # 選擇後才獲取參數
            top_n, min_days, sort_by_amount = _get_user_params()
