...
def market_scan_menu():
    ...
    while True:
        ...
        print_flush("[7] 重新載入指標")
        print_flush("[0] 返回主選單")
        
        ch = read_single_key()
        
        # 檢查快取
        if ch in ['1', '2', '3', '4', '5', '6'] and not GLOBAL_INDICATOR_CACHE["data"]:
            print_flush("\n正在載入指標...")
...
def step6_verify_and_backfill(data=None, resume=False):
    ...
    print_flush(f"\n✓ 資料回補完成 | 成功: {success_count} | 失敗: {fail_count} | 跳過: {skip_count}")
    return updated_codes

def _worker_calc_indicators(code):
    """[Worker] 單一股票指標計算 (用於多執行緒)"""
    try:
        # 每個執行緒獨立計算，calculate_stock_history_indicators 內部會建立自己的 DB 連線
        # 只要 db_manager.get_connection() 是 thread-safe (每次 return new connection) 即可
        indicators_list = calculate_stock_history_indicators(code, display_days=None)
        return code, indicators_list
    except Exception as e:
        return code, None

def step7_calc_indicators(data=None, target_codes=None, force=False):
    """重算技術指標 (含回補後數據)
    Args:
        force (bool): 是否強制重算 (忽略已存在的數據檢查)
    """
    print_flush("\n[Step 7] 重算技術指標 (含回補後數據)...")
    if force:
        print_flush("⚠ 強制重算模式: 將忽略所有已存在的數據檢查")
    
    # Smart Skip Logic: If target_codes provided but empty -> Skip ALL
    if target_codes is not None:
        if not target_codes:
            print_flush("✓ 無需重新計算 (無新資料)")
            return data
        else:
            print_flush(f"ℹ 偵測到 {len(target_codes)} 檔股票更新，將重新計算所有股票指標...")
    ...
    # [優化] 使用多執行緒並行計算 (Parallel Processing)
    # 建議設為 CPU 核心數的 2 倍 (I/O Bound)
    # 限制最大 16 執行緒以避免 SQLite 鎖定問題
    max_workers = min(16, (os.cpu_count() or 4) * 4)
    
    print_flush(f"🚀 啟動並行計算 (Threads: {max_workers})...")
    ...
    with tracker:
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            # 提交所有任務
            future_to_code = {executor.submit(_worker_calc_indicators, code): code for code in codes}
            ...
            for i, future in enumerate(as_completed(future_to_code)):
                ...
                try:
                    _, indicators_list = future.result()
                    
                    if indicators_list:
                        # 將結果轉換為 tuple 格式並加入緩衝區
